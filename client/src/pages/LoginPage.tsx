import { useState, useEffect } from 'react'
import { Link, useNavigate, useSearchParams } from 'react-router-dom'
import { Button } from '@/components/ui/Button'
import { Logo } from '@/components/ui/Logo'
import { useAuth } from '@/contexts/AuthContext'
import { api } from '@/lib/api'
import { useRecaptcha } from '@/hooks/useRecaptcha'

export function LoginPage({ initialMode = 'login' }: { initialMode?: 'login' | 'signup' }) {
  const { login, signup } = useAuth()
  const navigate = useNavigate()
  const [params] = useSearchParams()
  const [mode, setMode] = useState<'login' | 'signup'>(initialMode)
  const [email, setEmail] = useState('')
  const [password, setPassword] = useState('')
  const [name, setName] = useState('')
  const [error, setError] = useState(params.get('error') === 'auth_failed' ? 'Google sign-in failed' : '')
  const [loading, setLoading] = useState(false)
  const [googleEnabled, setGoogleEnabled] = useState(true)
  const { getToken } = useRecaptcha(mode === 'login' ? 'login' : 'signup')

  useEffect(() => {
    if (params.get('mode') === 'signup') setMode('signup')
  }, [params])

  useEffect(() => {
    api<{ googleEnabled: boolean }>('/api/auth/config').then((res) => {
      if (res.success && res.data) setGoogleEnabled(res.data.googleEnabled)
    })
  }, [])

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setError('')
    setLoading(true)
    const captchaToken = await getToken()

    const err = mode === 'login'
      ? await login(email, password, captchaToken)
      : await signup(email, password, name, captchaToken)

    setLoading(false)
    if (err) {
      setError(err)
      return
    }
    const redirect = params.get('redirect')
    navigate(redirect && redirect.startsWith('/') ? redirect : '/account')
  }

  return (
    <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center px-6 py-16">
      <div className="w-full max-w-md">
        <div className="mb-10 flex justify-center">
          <Logo className="h-12 w-[100px]" />
        </div>

        <h1 className="mb-2 text-center text-2xl font-black uppercase tracking-tight">
          {mode === 'login' ? 'Welcome Back' : 'Create Account'}
        </h1>
        <p className="mb-8 text-center text-sm text-brand-muted">
          Men&apos;s fashion. Two sides. One identity.
        </p>

        {googleEnabled && (
          <>
            <a
              href="/api/auth/google"
              className="mb-6 flex w-full items-center justify-center gap-3 border border-white/20 px-6 py-3 text-xs font-semibold uppercase tracking-widest transition-colors hover:bg-white/5"
            >
              <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
                <path fill="#fff" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"/>
                <path fill="#fff" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#fff" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                <path fill="#fff" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
              </svg>
              Continue with Google
            </a>

            <div className="mb-6 flex items-center gap-4">
              <div className="h-px flex-1 bg-white/10" />
              <span className="text-[10px] uppercase tracking-widest text-brand-muted">or</span>
              <div className="h-px flex-1 bg-white/10" />
            </div>
          </>
        )}

        <form onSubmit={handleSubmit} className="space-y-4">
          {mode === 'signup' && (
            <input
              type="text"
              placeholder="Full name"
              value={name}
              onChange={(e) => setName(e.target.value)}
              required
              className="w-full border border-white/20 bg-transparent px-4 py-3 text-sm focus:outline-none focus:border-white/50"
            />
          )}
          <input
            type="email"
            placeholder="Email"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            required
            autoComplete="email"
            className="w-full border border-white/20 bg-transparent px-4 py-3 text-sm focus:outline-none focus:border-white/50"
          />
          <input
            type="password"
            placeholder="Password"
            value={password}
            onChange={(e) => setPassword(e.target.value)}
            required
            minLength={8}
            autoComplete={mode === 'login' ? 'current-password' : 'new-password'}
            className="w-full border border-white/20 bg-transparent px-4 py-3 text-sm focus:outline-none focus:border-white/50"
          />

          {error && <p className="text-sm text-red-400">{error}</p>}

          <Button type="submit" variant="solid" fullWidth disabled={loading}>
            {loading ? 'Please wait...' : mode === 'login' ? 'Sign In' : 'Create Account'}
          </Button>
        </form>

        <p className="mt-6 text-center text-xs text-brand-muted">
          {mode === 'login' ? "Don't have an account? " : 'Already have an account? '}
          <button
            type="button"
            onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); setError('') }}
            className="text-brand-white underline underline-offset-2 hover:text-brand-light"
          >
            {mode === 'login' ? 'Sign up' : 'Sign in'}
          </button>
        </p>

        <p className="mt-4 text-center">
          <Link to="/" className="text-[10px] uppercase tracking-widest text-brand-muted hover:text-brand-white">
            ← Back to store
          </Link>
        </p>
      </div>
    </div>
  )
}
